"""Train MoMask's residual transformer (base tokens -> residual layers) on sign data.

Mirrors train_t2m_sign.py but uses ResidualTransformer and feeds the FULL
code_idx (all Q layers). mBART text encoder, HumanML3D eval removed, val-CE
early stopping.

    python train_res_sign.py --dataset phix --vq_name momask_vq_phix \
        --name momask_rtrans_phix --text mbart:de_DE --total_iter 60000
"""
...
def main():
    ...
    topt = SimpleNamespace(num_tokens=opt.nb_code, num_quantizers=opt.num_quantizers, device=device)
    trans = ResidualTransformer(code_dim=opt.code_dim, cond_mode='text', latent_dim=opt.latent_dim,
                                ff_size=opt.ff_size, num_layers=opt.n_layers, num_heads=opt.n_heads,
                                dropout=opt.dropout, clip_dim=1024, shared_codebook=False,
                                share_weight=False, cond_drop_prob=opt.cond_drop_prob,
                                clip_version=opt.text, opt=topt).to(device)
    print('res-transformer trainable params: %.2fM' %
    ...
    trans.train(); t0 = time.time(); run = {'loss': 0, 'acc': 0}
    for it in range(1, opt.total_iter + 1):
        ...
        with torch.no_grad():
            code_idx, _ = vq.encode(motion)
        loss, _pred, acc = trans(code_idx, list(cap), mlen // 4)
