"""Train Multi-Stream Residual VQ-VAE (M1+M2 combined)."""
...
def get_args():
    ...
    p.add_argument('--exp-name', default='vq_msr_debug')
    ...
    # base + residual per stream
    p.add_argument('--nb-base-body', type=int, default=64)
    p.add_argument('--nb-base-hand', type=int, default=128)
    p.add_argument('--nb-base-face', type=int, default=256)
    p.add_argument('--nb-res-body', type=int, default=64)
    p.add_argument('--nb-res-hand', type=int, default=128)
    p.add_argument('--nb-res-face', type=int, default=256)
...
def main():
    ...
    train_set = VQSignDataset(args.dataname, split='train', window_size=args.window_size)
    dev_set = VQSignDataset(args.dataname, split='dev', window_size=args.window_size,
    ...
    stream_codes_base = {'body': args.nb_base_body, 'hand': args.nb_base_hand, 'face': args.nb_base_face}
    stream_codes_res = {'body': args.nb_res_body, 'hand': args.nb_res_hand, 'face': args.nb_res_face}
    model = MultiStreamResidualVQVAE(
        args, dataset_name=args.dataname,
        code_dim=args.code_dim, output_emb_width=args.output_emb_width,
    ...
    log(f'[*] MSR-VQ-VAE: streams={list(model.stream_dims.keys())}, '
        f'base={stream_codes_base}, res={stream_codes_res}, '
