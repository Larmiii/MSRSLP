...
class VQVAE_251(nn.Module):
    def __init__(self,
        ...
        # Dataset → input dim mapping
        input_dim_map = {
            'kit': 251,
            't2m': 263,
            'phix': 534,      # 178 keypoint × 3 (Mediapipe subset, SLRTP data)
            'csl':  399,      # 133 keypoint × 3 (HRNet WholeBody)
            'phix14t': 399,   # 133 keypoint × 3 (HRNet, MSKA paper data)
            'csl_lift3d': 534,# 178 keypoint × 3 (Ivashechkin 3D lift on CSL-Daily)
            'phix_lift3d': 534, # 178 keypoint × 3 (SLRTP-178 lift on PHIX-14T)
        }
        _name = getattr(args, 'dataname', 't2m')
        input_dim = input_dim_map.get(_name, getattr(args, 'input_dim', 263))
