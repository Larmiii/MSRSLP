"""Train VQ-VAE on sign-language data (PHIX-14T / CSL-Daily).

Adapted from T2M-GPT's train_vq.py but simplified:
- Custom sign-language dataset (dataset_VQ_sign.py)
- Reconstruction loss + VQ commitment + velocity loss
- Periodic dev reconstruction MPJPE check
- No T2M HumanML3D evaluator (we use SLRTP/MSKA BT eval separately)

Usage:
    # PHIX baseline:
    python train_vq_sign.py --dataname phix --exp-name vq_phix_base \
        --batch-size 128 --nb-code 512 --total-iter 100000

    # CSL-Daily baseline:
    python train_vq_sign.py --dataname csl --exp-name vq_csl_base \
        --batch-size 128 --nb-code 512 --total-iter 100000
"""
...
sys.path.insert(0, str(Path(__file__).resolve().parent))   # T2M_GPT dir on path
...
def get_args():
    ...
    p.add_argument('--exp-name', type=str, default='vq_debug')
    ...
    p.add_argument('--save-iter', type=int, default=10000)
    ...
    p.add_argument('--early-stop-patience', type=int, default=4,
                    help='stop if dev recon has not improved for this many eval intervals')
    p.add_argument('--early-stop-min-delta', type=float, default=0.001,
                    help='min dev recon improvement to count as progress')
    p.add_argument('--min-iter-before-early-stop', type=int, default=5000,
                    help='no early stopping before this iter (let warmup + initial training finish)')
    return p.parse_args()
...
@torch.no_grad()
def eval_dev_recon(model, dev_loader, device):
    model.eval()
    total_recon = 0.0; total_vq = 0.0; n = 0
    for batch in dev_loader:
        batch = batch.to(device)
        out, vq_loss, ppl = model(batch)
        recon = (out - batch).pow(2).mean()
        bs = batch.size(0); n += bs
        total_recon += recon.item() * bs
        total_vq += vq_loss.item() * bs
    model.train()
    return total_recon / max(n, 1), total_vq / max(n, 1)
...
def main():
    ...
    dev_loader = DataLoader(dev_set, batch_size=args.batch_size,
                              ...
                              pin_memory=True)
    log(f'[*] train: {len(train_set)} samples, dev: {len(dev_set)}')

    # Save mean/std for inference reuse
    ...
    log(f'[*] VQ-VAE: input_dim={model.input_dim}, '
    ...
    Loss = losses_lib.ReConsLoss(args.recons_loss, model.input_dim // 3)
    ...
    while it < args.total_iter:
        ...
        out, vq_loss, perplexity = model(batch)
        ...
        if it % args.print_iter == 0:
            log(f'iter {it:>6d} | lr {cur_lr:.5f} | '
                f'recon {recon_loss.item():.4f} | vel {vel_loss.item():.4f} | '
                f'vq {vq_loss.item():.4f} | ppl {perplexity.item():.1f} | '
        ...
        if it > 0 and it % args.eval_iter == 0:
            ...
            if improved:
                ...
                no_improve_count += 1
                log(f'    >> no improvement ({no_improve_count}/{args.early_stop_patience}), '
                    f'best still {best_dev_recon:.4f} @ iter {best_iter}')
                # Early stopping check
                if (it >= args.min_iter_before_early_stop and
                        no_improve_count >= args.early_stop_patience):
                    log(f'    >> EARLY STOP at iter {it} '
                        f'(no improvement for {no_improve_count} evals)')
                    break
    ...
    torch.save({'model': model.state_dict(), 'iter': it, 'args': vars(args)},
                out_dir / 'final.pt')
    log(f'[OK] training done in {(time.time()-t0)/60:.1f} min')
    log(f'best dev recon: {best_dev_recon:.4f}')
    log_f.close()
