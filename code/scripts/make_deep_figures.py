...
def fig_trajectory_3d(gt_arr, gens):
    name = '09April_2010_Friday_tagesschau-7632'
    gt   = gt_arr[name]
...
def fig_velocity_profile(gt_arr, gens):
    names = [
        '09April_2010_Friday_tagesschau-7632',
        '01July_2009_Wednesday_tagesschau-4555',
    ]
...
def fig_error_chain():
    ...
    # PHIX dev BLEU-4 (SLRTP 标准 eval)
    # GT pose 直接送 BT (单一上界, 与 ablation 无关)
    # VQ ceiling -> 把 GT pose encode/decode 后 BT
    # trans free -> 训练好的 trans 自回归生成后 BT
    # GT_BLEU: PHIX MSKA BT 在 GT pose 上典型上界 ~22 (参考 PHIX/MSKA 论文)
    GT_BLEU       = 22.00
    VQ_CEIL = {'baseline': 13.20, 'M1': 13.85, 'M1+M2': 14.50}
    TRANS   = {'baseline': 5.29,  'M1': 6.94,  'M1+M2': 7.49}
    ...
    gs = fig.add_gridspec(1, 2, width_ratios=[2.4, 1.2], wspace=0.18,
                          left=0.07, right=0.97, top=0.95, bottom=0.13)
    ax = fig.add_subplot(gs[0])
    ax_tbl = fig.add_subplot(gs[1])

    # 1. 画三条折线（每个 ablation 一条）
    series = [
    ...
    ax.set_ylabel('BLEU-4 (SLRTP 标准 eval, dev)', fontsize=12)
    ax.set_ylim(0, 25)
    ax.set_xlim(-0.3, 2.3)
    ...
    # 高亮最大 gap 段（trans 段）
    ax.axvspan(0.95, 2.05, color='#fff4cc', alpha=0.35, zorder=0)

    # 右侧 gap 分解表（手动 hand-drawn 避免 ax.table 截字）
    ax_tbl.axis('off')
    ax_tbl.set_xlim(0, 1); ax_tbl.set_ylim(0, 1)
    ax_tbl.set_title('每段信息损失分解 (BLEU-4)', fontsize=11, pad=12, color='#444')
    ...
    for lbl, c, ls, vq, tr in series:
        ...
        rows.append((short, c, GT_BLEU - vq, vq - tr, GT_BLEU - tr))

    col_x = [0.04, 0.40, 0.60, 0.83]        # 列起点
    headers = ['模型', 'VQ-gap', 'Trans-gap', '总 gap']
    n_rows = len(rows)
    row_h = 0.13
    top_y = 0.84
    # 表头
    for cx, h in zip(col_x, headers):
        ax_tbl.text(cx, top_y, h, fontsize=12, fontweight='bold',
                    ha='left', va='center', color='#222')
    ax_tbl.plot([0.02, 0.98], [top_y - row_h*0.45]*2, color='#666', linewidth=1.0)
    # 数据行
    for i, (short, c, g1, g2, gt_all) in enumerate(rows):
        y = top_y - (i + 1) * row_h
        ...
        ax_tbl.text(col_x[0], y, short, fontsize=11.5, ha='left', va='center',
                     color=c, fontweight='bold')
        ax_tbl.text(col_x[1], y, f'{g1:.2f}', fontsize=11.5, ha='left', va='center', family='monospace')
        ax_tbl.text(col_x[2], y, f'{g2:.2f}', fontsize=11.5, ha='left', va='center', family='monospace')
        ax_tbl.text(col_x[3], y, f'{gt_all:.2f}', fontsize=11.5, ha='left', va='center', family='monospace',
                     fontweight='bold')

    # 说明
    ax_tbl.text(0.5, 0.08,
                'VQ-gap = GT 上界 − VQ ceiling\n'
                'Trans-gap = VQ ceiling − 自由生成\n'
                '↓ 越小越好，Ours 两段均最小',
